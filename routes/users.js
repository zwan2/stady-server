...
/* /stopwatch로 이전했음
//REQ: userId RES: arr[], today_goal, subject_goals
router.get('/loadMain', function (req, res, next) {
  var querySelectGoal = "SELECT today_goal, subject_goals FROM user_goals WHERE user_id = ? AND exam_address = (SELECT exam_address FROM user_settings d WHERE d.user_id = ?) ORDER BY reg_time DESC LIMIT 1";
