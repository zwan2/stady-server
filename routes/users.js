...
//유저의 골, 최근 정보 가져오기
router.get('/loadGoalHistory', function (req, res, next) {
  var querySelectGoal = "SELECT goal_setting FROM user_goals WHERE user_id = ? AND exam_address = ?";
  var querySelectHistory =  "SELECT subject_id, study_id, term FROM histories WHERE user_id = ? AND exam_address = ? AND start_point > DATE_SUB(NOW(), INTERVAL 1 day)";
